...
chart-lint:
    ...
    # plugin. Unconnected, no token is projected and no outbox mounted;
    # connected, both workloads -- the service and the controller -- get
    # their own token for the installation's audience and an outbox; and
    # half a connection is refused at render, not at start-up.
    ! helm template access-issuer charts/access-issuer \
        --set issuerURL=https://access.example | grep -q 'audit-token'
    helm template access-issuer charts/access-issuer \
        --set issuerURL=https://access.example \
        --set 'exchange.clusters[0].name=example' \
        --set 'exchange.clusters[0].issuer=https://oidc.example' \
        --set console.mount=/console \
        --set githubRoster.enabled=true \
        --set audit.writer=http://audit-writer.audit.svc:8080 \
        --set audit.registry=http://audit-registry.audit.svc:8080 \
        --set audit.query=http://audit-query.audit.svc:8080 \
        > /tmp/access-issuer-audit.yaml
    test "$(grep -c 'name: audit-token$' /tmp/access-issuer-audit.yaml)" = "4"
    test "$(grep -c 'value: /var/lib/audit-outbox$' /tmp/access-issuer-audit.yaml)" = "2"
    grep -A1 'name: AUDIT_QUERY_URL' /tmp/access-issuer-audit.yaml | grep -q 'audit-query.audit.svc'
    ! helm template access-issuer charts/access-issuer \
        --set issuerURL=https://access.example \
        --set audit.writer=http://audit-writer.audit.svc:8080 2>/dev/null
...
check: build test lint chart-lint archive-check docs-check audit-catalogue ts vuln
