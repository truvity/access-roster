...
//
// The trail itself is somebody else's: an installation of
// github.com/truvity/audit, deployed on its own, which pseudonymises, locks,
// signs, indexes and serves what it is sent. access-roster connects to it as a
// plugin. It declares what it records in a catalogue (catalogue/roster.yaml),
// registers that catalogue at start-up, sends each record to the writer, and
// shows the trail on its console through the installation's query service.
// With no installation configured, nothing is kept beyond the log line every
// record also is.
//
...
// access outage, so almost every action is delivered through an outbox on
// local disk and reaches the writer when it can. A recovery sign-in bypasses
// the directory, so its catalogue entry says `block`: [Trail.RecordDurable]
// returns only once the installation has it, and the caller refuses the
