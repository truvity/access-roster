...
| `route.pathPrefix` | `""` | mount the console under a path of the host — `/console` — with the gateway rewriting the prefix away, so the hub's own routes do not change. This is how the console shares its issuer's hostname: the issuer must sit at the origin root (its `iss` claim and discovery are there), so the console takes the path. `PUBLIC_URL` and every redirect URI built from it carry the prefix; `bootstrapPaths` are matched under it. Empty is today's shape *(INF-687)* |
...
| `PUBLIC_URL` | `https://<route.host><route.pathPrefix>` — where a browser reaches the console. **Both OAuth redirect URIs and the setup values are built from it**, so a deployment without `route.host` falls back to localhost and registers a redirect no browser will reach |
