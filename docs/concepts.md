...
| **secret store** | an OpenBAO (or Vault) the deployment declares in `secretManagers`, shown on the console's Secret stores page (`/secret-stores`): its namespaces, the groups each admits, what those groups open, and what one person can reach. Read-only, and not by omission — the store's desired state lives in the estate's repository, so a drift row links to where the fix is made and is never a button. A namespace **is** an environment; a group is `bound`, `not applied yet`, `not declared` or `cannot read`, and the last is never inferred from an empty answer |
