...
| Providers, and one page per provider | which tenants we read, their domains and standing, the actions on the tenant itself, and the groups and accounts it holds — every one a link. One row per *(provider, domain)* pair, repeating the provider, because the row is the unit a reader compares and a stack of domains inside one cell cannot be scanned down. **Add a provider** offers both ways in, admin consent and an uploaded key, and only the ways this deployment can take |
| Groups (identity side), and one page per group | what the providers say exists, and which of it the policy uses, filterable by provider and by domain. A group's page reads along the chain: its members as the provider reports them, the internal groups it feeds, the clients that therefore open |
| People, and one page per person | every account, as the last snapshot has it, filtered by provider, by domain and by whether it is live. The domain filter is applied by the hub and not over the page the console received: this list is capped at 200, and narrowing that page in the browser would answer *nobody* while the snapshot holds hundreds. A person's page is where the two sides meet: their directory groups, then the chain one row per internal group held — what put them in it and what it opens — then what did not open and why. Once the issuer exists it gains **Active sessions** with Revoke — one row per application, grouped under the browser session they were opened from — and your own page **Sign out everywhere** |
...
| Groups (access side), and one page per group | the vocabulary of access. A group's page mirrors a provider group's: the provider groups that feed it, the people that puts in it now, what it adds to a token, the clients it opens |
...
**The console says *provider*.** What this document, the policy and the
code call a *workspace* or a *directory*, the console calls a
**provider**, and the group it holds a **provider group**. Both sides of
the rail then say simply **Groups**, and the section heading above each
— *Where people come from*, *Internal* — is what tells them apart; the
reader is not made to carry a qualifier down every page. The URLs and
the API keep the old words, so nothing anyone has bookmarked or scripted
moves.
...
| sessions, once the issuer exists | revoke another identity's, a client's, or any in the installation (operator); sign out everywhere (anyone, their own). Removal only: a console can end access here, never grant it. The pages are the console's, same-origin with the issuer on one domain, and the browser calls the issuer's session service with its own session cookie — the hub's code stays independent of the issuer | — |
