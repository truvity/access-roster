...
| GitHub organisations | one controller App per org, connected like a directory; `github` bindings in the policy naming internal groups; a login matched to a person by verified-domain email | **the bindings are built** and the console lists them (INF-696); the controller that acts on them is not (INF-697) |
