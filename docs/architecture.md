...
| GitHub organisations | one controller App per org, connected like a directory; `github` bindings in the policy naming internal groups; a login matched to a person by verified-domain email | **built** (INF-696, INF-697): the bindings, the GitHub page, Connect, and the controller beside the service; not yet enabled against a real organisation |
