...
func TestAWSCacheLockMintsOnce(t *testing.T) {
	path := cacheFile(t)
	...
	for range 8 {
		...
		go func() {
			defer wg.Done()
			_ = withAWSCacheLock(path, func() error {
				if _, ok := readAWSCache(path); ok {
...
func TestAWSCacheLockIsASeparateFile(t *testing.T) {
	path := cacheFile(t)
	if err := withAWSCacheLock(path, func() error { return nil }); err != nil {
